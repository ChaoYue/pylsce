...
import pandas as pa
import pb
from glccIndexTree6Herb3 import *
...
def vegmax_SinglePoint_to_veg4type(vegmax):
    ...
    g = vegmax[ia10_1:ia10_3+1].sum() + vegmax[ia12_1:ia12_3+1].sum()
    p = vegmax[ia11_1:ia11_3+1].sum() + vegmax[ia13_1:ia13_3+1].sum()
    c = vegmax[ia14_1:ia15_3+1].sum()
...
def glccpftmtc_SinglePoint_to_4X4matrix(glcc_pftmtc):
    ...
    glccReal_from_glccpftmtc[1,0] = glcc_pftmtc[ia10_1:ia10_3+1,1:9].sum() + glcc_pftmtc[ia12_1:ia12_3+1,1:9].sum()
    glccReal_from_glccpftmtc[1,2] = glcc_pftmtc[ia10_1:ia10_3+1,[10,12]].sum() + glcc_pftmtc[ia12_1:ia12_3+1,[10,12]].sum()
    glccReal_from_glccpftmtc[1,3] = glcc_pftmtc[ia10_1:ia10_3+1,[13,14]].sum() + glcc_pftmtc[ia12_1:ia12_3+1,[13,14]].sum()

    glccReal_from_glccpftmtc[2,0] = glcc_pftmtc[ia11_1:ia11_3+1,1:9].sum() + glcc_pftmtc[ia13_1:ia13_3+1,1:9].sum()
    glccReal_from_glccpftmtc[2,1] = glcc_pftmtc[ia11_1:ia11_3+1,[9,11]].sum() + glcc_pftmtc[ia13_1:ia13_3+1,[9,11]].sum()
    glccReal_from_glccpftmtc[2,3] = glcc_pftmtc[ia11_1:ia11_3+1,[13,14]].sum() + glcc_pftmtc[ia13_1:ia13_3+1,[13,14]].sum()

    glccReal_from_glccpftmtc[3,0] = glcc_pftmtc[ia14_1:ia15_3+1,1:9].sum()
    glccReal_from_glccpftmtc[3,1] = glcc_pftmtc[ia14_1:ia15_3+1,[9,11]].sum()
    glccReal_from_glccpftmtc[3,2] = glcc_pftmtc[ia14_1:ia15_3+1,[10,12]].sum()
...
def get_veget_fraction_vegetmax(vegmax):
    ...
    newdic['forest'] = dic['Forest_Age_1'] + dic['Forest_Age_2'] + dic['Forest_Age_3'] + dic['Forest_Age_4'] + dic['Forest_Age_5'] \
                       + dic['Forest_Age_6']
    newdic['grass'] = dic['Grassland_Age_1'] + dic['Grassland_Age_2'] + dic['Grassland_Age_3']
    newdic['pasture'] = dic['Pasture_Age_1'] + dic['Pasture_Age_2'] + dic['Pasture_Age_3']
    newdic['crop'] = dic['Crop_Age_1'] + dic['Crop_Age_2'] + dic['Crop_Age_3']
    return newdic
