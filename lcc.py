...
def glccpftmtc_SinAgeC_SinglePoint_to_4X4matrix(glcc_pftmtc):
    """
    Convert the glcc_pftmtc(65x15) to 4x4 matrix, in a dataframe:
    Column = recieving; Row = giving
        f   g   p   c
    ...
    glccReal_from_glccpftmtc[0,0] = glcc_pftmtc[1:9,1:9].sum()
    glccReal_from_glccpftmtc[0,1] = glcc_pftmtc[1:9,[9,11]].sum()
    glccReal_from_glccpftmtc[0,2] = glcc_pftmtc[1:9,[10,12]].sum()
    glccReal_from_glccpftmtc[0,3] = glcc_pftmtc[1:9,[13,14]].sum()

    glccReal_from_glccpftmtc[1,0] = glcc_pftmtc[9,1:9].sum() + glcc_pftmtc[11,1:9].sum()
    glccReal_from_glccpftmtc[1,2] = glcc_pftmtc[9,[10,12]].sum() + glcc_pftmtc[11,[10,12]].sum()
    glccReal_from_glccpftmtc[1,3] = glcc_pftmtc[9,[13,14]].sum() + glcc_pftmtc[11,[13,14]].sum()

    glccReal_from_glccpftmtc[2,0] = glcc_pftmtc[10,1:9].sum() + glcc_pftmtc[12,1:9].sum()
    glccReal_from_glccpftmtc[2,1] = glcc_pftmtc[10,[9,11]].sum() + glcc_pftmtc[12,[9,11]].sum()
    glccReal_from_glccpftmtc[2,3] = glcc_pftmtc[10,[13,14]].sum() + glcc_pftmtc[12,[13,14]].sum()

    glccReal_from_glccpftmtc[3,0] = glcc_pftmtc[13:15,1:9].sum()
    glccReal_from_glccpftmtc[3,1] = glcc_pftmtc[13:15,[9,11]].sum()
    glccReal_from_glccpftmtc[3,2] = glcc_pftmtc[13:15,[10,12]].sum()
...
def get_age_fraction_vegetmax(vegmax):
    ...
    frac_F['Forest_Age_1'] = vegmax[:,np.arange(ia2_1,ia9_5,5),...].sum(axis=1)
    frac_F['Forest_Age_2'] = vegmax[:,np.arange(ia2_2,ia9_5,5),...].sum(axis=1)
    frac_F['Forest_Age_3'] = vegmax[:,np.arange(ia2_3,ia9_5,5),...].sum(axis=1)
    frac_F['Forest_Age_4'] = vegmax[:,np.arange(ia2_4,ia9_5,5),...].sum(axis=1)
    frac_F['Forest_Age_5'] = vegmax[:,np.arange(ia2_5,ia9_5+1,5),...].sum(axis=1)
